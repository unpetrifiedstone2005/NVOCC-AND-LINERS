...
export function ContainerComponent() {
  const [activeTab, setActiveTab] = useState<
    "container-types" | "containers" | "bulk-import" | "container-list"
  >("container-types");
  ...
  const [bulkImportMode, setBulkImportMode] = useState<"textarea" | "file">(
    "textarea"
  );
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(({ data }) => {
        ...
        setContainerCount(
          typeof data.total === "number" ? data.total : data.containers.length
        );
      })
      .catch((err) => {
  ...
  const handleSubmitContainerType = async (e: React.FormEvent) => {
    ...
    try {
      // 1) POST the new type
      const { data: newType } = await axios.post<ContainerType>(
        "/api/seed/containers/types/post",
        containerTypeForm
      );
      ...
      if (err.response?.data?.errors) {
        ...
        showMessage(
          "error",
          err.response?.data?.error || "Failed to create container type"
        );
      }
    } finally {
      setIsLoading(false);
  ...
  const handleBulkImport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showMessage(
        "error",
        "Invalid JSON format. Provide an array of container objects."
      );
      setIsLoading(false);
      return;
    }
    ...
    items.forEach((c, i) => {
      if (!c.containerNo || !c.containerTypeIsoCode) {
        errors.push(
          `Item ${i + 1}: missing containerNo or containerTypeIsoCode`
        );
      }
    });
    if (errors.length) {
    ...
    if (successCount) {
      ...
      showMessage(
        "success",
        `Imported ${successCount} containers${
      ...
      showMessage(
        "error",
        `No containers created. Errors: ${failed.join("; ")}`
      );
    }
  ...
  const downloadSampleJSON = () => {
    ...
    a.href = url;
    a.download = "container_import_sample.json";
    a.click();
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          ...
          const errors: string[] = [];
          rows.forEach((row, i) => {
            if (!row.containerNo || !row.containerTypeIsoCode) {
              errors.push(
                `Row ${i + 1}: missing containerNo or containerTypeIsoCode`
              );
            }
          });
          ...
          for (const row of rows) {
            try {
              const payload = {
                ...
                bicCode: row.bicCode || undefined,
                ownership: row.ownership || undefined,
                companyOrigin: row.companyOrigin || undefined,
                manufacturer: row.manufacturer || undefined,
                status: row.status || undefined,
                currentDepot: row.currentDepot || undefined,
                foodGrade: row.foodGrade === "true" || row.foodGrade === true,
              ...
              const msg =
                ...
                (Array.isArray(err.response?.data?.errors)
                  ? err.response.data.errors
                      ...
                      .join(", ")
                  : err.message);
              failed.push(`${row.containerNo || "[no number]"}: ${msg}`);
            }
          }
          ...
          if (successCount) {
            ...
            setMessage({
              type: "success",
              text: `Imported ${successCount} containers${
                failed.length ? `. Failed: ${failed.length}` : ""
              }`,
            });
          } else {
            setMessage({
              type: "error",
              text: `No containers created. Errors: ${failed.join("; ")}`,
            });
          }
        } catch (err) {
          console.error(err);
    ...
    <div className="w-full max-w-[1600px] mx-auto min-h-screen text-white uppercase">
      ...
      <header className="py-1 px-6 md:px-16">
        <div className="text-center">
          ...
          <p className="text-lg text-slate-100 mb-4 max-w-xl leading-relaxed mx-auto">
            Create and manage container types and individual containers for your
            maritime operations.
          </p>
        </div>
      </header>
      ...
      {activeTab === "container-types" && (
        <section className=" px-6 md:px-16">
          <div
            className=" rounded-3xl border-white border-2 shadow-[30px_30px_0px_rgba(0,0,0,1)] p-8 transition-shadow"
            style={cardGradientStyle}
          >
            <h2 className="text-3xl font-bold mb-8 flex items-center gap-3">
              <Package className="w-8 h-8 text-cyan-400" /> Create Container
              Type
            </h2>
            <form
              onSubmit={handleSubmitContainerType}
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const found = STANDARD_CONTAINER_TYPES.find(
                ...
                <input
                  ...
                  value={
                    STANDARD_CONTAINER_TYPES.find(
                      (t) => t.isoCode === containerTypeForm.isoCode
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setContainerTypeForm((prev) => ({
                      ...prev,
                      lengthMm: parseInt(e.target.value) || 0,
                    }))
                  }
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setContainerTypeForm((prev) => ({
                      ...prev,
                      tareWeightKg: parseInt(e.target.value) || 0,
                    }))
                  }
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setContainerTypeForm((prev) => ({
                      ...prev,
                      maxGrossWeightKg: parseInt(e.target.value) || 0,
                    }))
                  }
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setContainerTypeForm((prev) => ({
                      ...prev,
                      maxStackWeightKg: parseInt(e.target.value) || 0,
                    }))
                  }
                  className="w-full px-4 py-3 bg-[#11235d] hover:text-[#00FFFF] hover:bg-[#1a307a] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
      ...
      {activeTab === "containers" && (
        <section className="px-6 md:px-16">
          ...
          >
            <h2 className="text-3xl font-bold mb-8 flex items-center gap-3">
              <ContainerIcon className="w-8 h-8 text-cyan-400" /> Create
              Container
            </h2>
            <form
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setContainerForm((prev) => ({
                      ...prev,
                      customsApproval: e.target.value,
                    }))
                  }
                  className="w-full px-4 py-3 bg-[#11235d] hover:text-[#00FFFF] hover:bg-[#1a307a] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
        ...
        <section className="px-6 md:px-16 mb-16">
          ...
          >
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.files?.[0]) {
                  handleCSVImport(e.target.files[0]).then(() => {
      ...
      {activeTab === "container-list" && (
        <section className="px-6 md:px-16">
          <div
            className=" rounded-3xl shadow-[0px_30px_0px_rgba(0,0,0,1)] p-8 border-white border-2"
            style={cardGradientStyle}
          >
            <h2 className="text-3xl font-bold mb-8 flex items-center gap-3">
            ...
            <div
              ...
              style={cardGradientStyle}
            >
              <h3 className="text-lg font-semibold text-[#00FFFF] mb-4 ">
                Filters
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFilterChange("containerTypeIsoCode", e.target.value)
                    }
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleFilterChange("currentDepot", e.target.value)
                    }
            ...
            {isLoadingList ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-md font-semibold text-white mt-2">
                  Create some containers first to see them listed here
                </p>
              </div>
            ) : (
              <>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="text-white font-semibold">
                            {container.containerTypeIsoCode}
                          </span>
                        </div>
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-between">
                    <div className="text-sm text-slate-300">
                      Page {currentPage} of {totalPages} (showing {pageSize} per
                      page)
                    </div>
                    <div className="flex gap-3">
                      <button
      ...
      <section className="py-16 px-6 md:px-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl mx-auto">
          <div
            className="bg-[#1A2A4A] rounded-2xl border border-slate-700 shadow-[10px_10px_0px_rgba(0,0,0,1)] text-center px-8 py-6 flex flex-col items-center"
            style={cardGradientStyle}
          >
      ...
      {isEditModalOpen && selectedContainer && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div
            className="bg-[#121c2d] rounded-3xl border-2 border-white shadow-[30px_30px_0px_rgba(0,0,0,1)] p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto"
            style={cardGradientStyle}
          >
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-3xl font-bold flex items-center gap-3">
                <Edit3 className="w-8 h-8 text-cyan-400" /> Edit Container:{" "}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("containerNo", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                  placeholder="SCMT1234567"
                />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("containerTypeIsoCode", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                  placeholder="40HC"
                />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("bicCode", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                  placeholder="SCMT"
                />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("ownership", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                  placeholder="SCMT"
                />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("currentDepot", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                  placeholder="Basra Port"
                />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("companyOrigin", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                  placeholder="Iraq"
                />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("customsApproval", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                  placeholder="Approved"
                />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("cscPlateUrl", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                  placeholder="https://example.com/plate.jpg"
                />
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("certificationExpiry", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                />
              </div>
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("lastUsedAt", e.target.value)
                  }
                  className="w-full px-4 py-3 bg-[#1d4595] hover:bg-[#1A2A4A] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
                />
              </div>
