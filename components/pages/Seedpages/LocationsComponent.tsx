...
export function LocationsComponent() {
  ...
  function downloadSample() {
    const sample = [
      // PORTS (need UN/LOCODE)
      {
        ...
        doorNotes: "Full inland coverage",
      ...
      },
      {
        ...
        country: "IE",
        type: "PORT",
        doorPickupAllowed: false,
        doorDeliveryAllowed: false,
        doorNotes: "CY only (no door)",
        aliases: ["DUBLIN PORT"],
      },
      ...
      },
      {
        ...
        country: "IN",
        type: "INLAND",
        doorPickupAllowed: null,
        doorDeliveryAllowed: null,
        doorNotes: "Unknown; verify locally",
    ...
    a.href = url;
    a.download = "locations-sample.json";
    a.click();
  }
  ...
  async function fetchLocations(page = 1) {
    ...
    try {
      ...
      const items: Location[] = (res.data.items as any[]).map(
        (x): Location => ({
          id: String(x.id),
          ...
          doorDeliveryAllowed: parseBoolish(
            x.doorDeliveryAllowed ?? x.deliveryDoor
          ),

          doorNotes:
            typeof x.doorNotes === "string" ? x.doorNotes : x.doorNotes ?? "",
    ...
    try {
      ...
      const payload = {
        unlocode: form.unlocode?.trim()
          ? form.unlocode.trim().toUpperCase()
          : null,
        name: form.name.trim(),
      ...
      complete: async (results) => {
        try {
          const data = results.data.map((row: any) => ({
            ...row,
            aliases: row.aliases
              ? row.aliases.split(",").map((a: string) => a.trim())
              : [],
          }));

          const res = await fetch("/api/seed/locations/post?onConflict=skip", {
            method: "POST",
  ...
  async function importBulk(e: React.FormEvent) {
    ...
    try {
      ...
      const normalized = rows.map((r, i) => {
        ...
        if (type === "SEAPORT" && !unlocode) {
          throw new Error(
            `Row ${
              i + 1
            }: UN/LOCODE (field "unlocode" or "code") is required for PORT/SEAPORT`
          );
        }
        if (unlocode && !/^[A-Z0-9]{5}$/.test(unlocode)) {
          throw new Error(
            `Row ${
              i + 1
            }: invalid UN/LOCODE "${unlocode}" (expected 5 alphanumerics, e.g. USLAX)`
          );
        }
      ...
      await axios.post(
        "/api/seed/locations/post?onConflict=update",
        normalized
      );
  ...
  const ports = [
    ...
    {
      code: "DMJ",
      name: "Port of Damman (King Abdul Aziz Port)",
      city: "Dammam",
    ...
    <div className="w-full max-w-[1600px] mx-auto min-h-screen text-white uppercase">
          ...
          >
            ...
            <div>
              ...
              >
                {ports.map((port) => (
                  ...
                  >
                    <div
                      ...
                      className="w-4 h-4 bg-blue-600 border-2 border-white rounded-full shadow-lg cursor-pointer"
                    />
                  </Marker>
                ))}
                ...
                {hoveredPort && (
                  <Popup
                    longitude={hoveredPort.longitude}
                    latitude={hoveredPort.latitude}
                    closeButton={false}
                    closeOnClick={false}
                    anchor="top"
                  >
                    <div>
                      <p className=" text-black">
                        <strong>{hoveredPort.name}</strong>
                      </p>
                      <p className=" text-black">Code: {hoveredPort.code}</p>
                      <p className=" text-black">City: {hoveredPort.city}</p>
                      <p className=" text-black">
                        Country: {hoveredPort.country}
                      </p>
                    </div>
                  </Popup>
                )}
            ...
            >
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold">
                  Door Pickup (from this location)
                </label>
                <select
                  ...
                  onChange={(e) =>
                    setForm((prev) => ({
                      ...prev,
                      pickupDoor: e.target.value as DoorTri,
                    }))
                  }
                  className="w-full px-4 py-3 bg-[#2D4D8B] border-4 border-black rounded-lg text-white focus:border-white mt-2"
                >
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold">
                  Door Delivery (to this location)
                </label>
                <select
                  ...
                  onChange={(e) =>
                    setForm((prev) => ({
                      ...prev,
                      deliveryDoor: e.target.value as DoorTri,
                    }))
                  }
                  className="w-full px-4 py-3 bg-[#2D4D8B] border-4 border-black rounded-lg text-white focus:border-white mt-2"
                >
              ...
              <div className="md:col-span-2 space-y-2">
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setForm((prev) => ({ ...prev, doorNotes: e.target.value }))
                  }
                  rows={3}
              ...
              <div className="md:col-span-2 space-y-2">
                ...
                <p className="text-xs text-white/90">
                  We’ll store these as uppercased tokens in{" "}
                  <code>aliases[]</code>.
                </p>
              </div>
        ...
        <section className="px-6 md:px-16 mb-16">
          ...
          >
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.files?.[0]) {
                  handleCSVUpload(e.target.files[0]).then(() => {
      ...
      {editModalOpen && selected && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div
            className="bg-[#121c2d] rounded-3xl p-6 max-w-md w-full"
            style={cardGradient}
          >
