  ...
  export function LocationsComponent() {
    ...
    function downloadSample() {
      const sample = [
        ...
        { unlocode: "INBOM", name: "Mumbai",   city: "Mumbai",   country: "IN", type: "PORT",   doorPickupAllowed: true,  doorDeliveryAllowed: true,  doorNotes: "Full inland coverage" },
        { unlocode: "IEDUB", name: "Dublin",   city: "Dublin",   country: "IE", type: "PORT",   doorPickupAllowed: false, doorDeliveryAllowed: false, doorNotes: "CY only (no door service)" },
        // INLAND CITIES (UN/LOCODE preferred but optional in our API)
        { unlocode: "USCHI", name: "Chicago",  city: "Chicago",  country: "US", type: "INLAND", doorPickupAllowed: true,  doorDeliveryAllowed: true },
        { unlocode: "INDEL", name: "Delhi",    city: "Delhi",    country: "IN", type: "INLAND", doorPickupAllowed: null,  doorDeliveryAllowed: null, doorNotes: "Unknown; verify locally" }
      ];
    ...
    async function createLocation(e: React.FormEvent) {
      ...
      try {
        ...
        // IMPORTANT: update on conflicts so type/door changes take effect
    ...
    async function applyEdit() {
      ...
      setIsUpdating(true);
      try {
        // Include door fields in the PATCH request
        await axios.patch(`/api/seed/locations/${selected.id}/patch`, {
          ...
          type:     editForm.type,
          // Include door capability fields
  ...
  async function importBulk(e: React.FormEvent) {
    ...
    try {
      ...
      const normalized = rows.map((r, i) => {
        ...
        return { unlocode, name, city, country, type, doorPickupAllowed, doorDeliveryAllowed, doorNotes };
      });
                ...
                {bulkMode === "file" ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      placeholder={`[
    { "unlocode": "INBOM", "name": "Mumbai",  "city": "Mumbai",  "country": "IN", "type": "PORT",   "doorPickupAllowed": true,  "doorDeliveryAllowed": true,  "doorNotes": "Full inland coverage" },
    { "unlocode": "IEDUB", "name": "Dublin",  "city": "Dublin",  "country": "IE", "type": "PORT",   "doorPickupAllowed": false, "doorDeliveryAllowed": false, "doorNotes": "CY only (no door)" },
    { "unlocode": "USCHI", "name": "Chicago", "city": "Chicago", "country": "US", "type": "INLAND", "doorPickupAllowed": true,  "doorDeliveryAllowed": true },
    { "unlocode": "INDEL", "name": "Delhi",   "city": "Delhi",   "country": "IN", "type": "INLAND", "doorPickupAllowed": null,  "doorDeliveryAllowed": null, "doorNotes": "Unknown; verify" }
  ]`}
                      value={bulkData}
                ...
                <div className="mt-6 flex gap-4 justify-end">
                  <button
                    onClick={() => {
                      ...
                      // If you want to re-fetch immediately:
                      // fetchLocations(1);
                    }}
