...
export function ServiceComponent() {
  const [activeTab, setActiveTab] = useState<
    "create-schedule" | "create-voyage" | "port-calls" | "bulk-import" | "schedule-list"
  >("create-schedule");
  ...
  async function createPortCall(e: React.FormEvent) {
    ...
    const match = formVoyages.find((v) => v.voyageNumber === voyageNumber && v.service?.id === scheduleId);
    if (!match) {
    ...
    setIsLoading(true);
    try {
      await axios.post(`/api/seed/serviceschedules/${scheduleId}/voyages/${voyageId}/portcalls/post`, {
  ...
  function openEditPortCall(pc: PortCall) {
    setPortCallEditForm(pc);
    setEditPortCallModal(true);
  }
  ...
  // === NEW: ensure schedules exist for tabs that need them (incl. Create Voyage)
  useEffect(() => {
    if (
      ["create-schedule", "create-voyage", "port-calls", "bulk-import"].includes(activeTab) &&
      !isLoadingSchedules &&
  ...
  const voyagesForSchedule = voyages.filter((v) =>
    v.voyageNumber?.toLowerCase().includes(debouncedVoyageSearch.toLowerCase())
  );
  const totalVoyagePages = Math.ceil(voyagesForSchedule.length / voyagesPerPage);
  const paginatedVoyages = voyagesForSchedule.slice(
    (voyagePage - 1) * voyagesPerPage,
    voyagePage * voyagesPerPage
  );
        ...
        <div
          className={`mx-6 mb-6 p-4 rounded-lg flex items-center gap-3 ${
            message.type === "success"
              ? "bg-green-900/30 border border-green-4 00 text-green-400"
        ...
        <section className="px-6 md:px-16 mb-16">
          <div className="rounded-3xl p-8 border-2 shadow-[30px_30px_0_rgba(0,0,0,1)]" style={cardGradient}>
            ...
            `}
              ...
              {/* Port Code */}
              <div className="space-y-2">
                <label htmlFor="portCode" className="text-sm font-semibold">
                  Port Code *
                </label>
                <input
                  id="portCode"
                  ...
                  onChange={(e) =>
                    setPortCallForm((prev) => ({ ...prev, portCode: e.target.value.toUpperCase() }))
                  }
                  placeholder="USNYC"
                  required
                  className="w-full px-4 py-3 bg-[#2D4D8B] hover:text-[#00FFFF] hover:bg-[#0A1A2F]
              ...
              <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label htmlFor="etd" className="text-sm font-semibold">
                    ETD *
                  </label>
                  <input
                    id="etd"
        ...
        <section className="px-6 md:px-16 mb-16">
          <div className="rounded-3xl p-8 border-2 shadow-[30px_30px_0_rgba(0,0,0,1)]" style={cardGradient}>
            ...
            <div className="bg-[#2e4972] rounded-lg p-6 mb-8 grid grid-cols-1 md:grid-cols-3 gap-10" style={cardGradient}>
              ...
              {/* 2nd column */}
              <div className="space-y-2">
                <label htmlFor="filter-desc" className="block text-sm font-semibold text-white">
                  Description
                </label>
                <input
                  id="filter-desc"
            ...
            <header className="flex items-center justify-between mb-6 gap-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={voyageSearch}
                  onChange={(e) => {
                    setVoyageSearch(e.target.value);
                    setVoyagePage(1);
                  }}
              ...
              <>
                {paginatedVoyages.length === 0 ? (
                  ...
                  hover:shadow-[19px_19px_0_rgba(0,0,0,1)] transition-shadow"
                      ...
                      >
                        ...
                        <div onClick={() => openEditVoyage(v)} className="relative flex items-center justify-between cursor-pointer">
                ...
                {totalVoyagePages > 1 && (
                  ...
                  disabled:cursor-not-allowed "
                    ...
                    <span className="text-sm text-white">Page {voyagePage} of {totalVoyagePages}</span>
                    <button
          ...
          <div className="bg-[#121c2d] rounded-3xl p-6 max-w-md w-full " style={cardGradient}>
            ...
            <div className="space-y-5">
              ...
              <input
                ...
                onChange={(e) =>
                  setPortCallEditForm({ ...portCallEditForm, portCode: e.target.value.toUpperCase() })
                }
                className="w-full px-4 py-3 bg-[#2D4D8B] hover:bg-[#0A1A2F] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] hover:shadow-[10px_8px_0_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
              />
              ...
              <input
                ...
                onChange={(e) =>
                  setPortCallEditForm({ ...portCallEditForm, order: parseInt(e.target.value) })
                }
                className="w-full px-4 py-3 bg-[#2D4D8B] hover:bg-[#0A1A2F] hover:text-[#00FFFF] mt-2 border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] hover:shadow-[10px_8px_0_rgba(0,0,0,1)] transition-shadow rounded-lg text-white placeholder-white/80 focus:border-white focus:outline-none"
              />
            ...
            <div className="flex justify-end gap-4 mt-4">
              ...
              >
                {isLoading ? <Settings className="animate-spin w-4 h-4" /> : <Save className="w-4 h-4" />}
