...
function groupForUiType(ui: string): {
  groupId: "DRY_STANDARD" | "DRY_HC";
  allowed: string[];
} {
  switch (ui) {
    case "20-general":
...
export const NewQuoteComponent: React.FC = () => {
  ...
  const [showModal, setShowModal] = useState<"breakdown" | "remarks" | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled)
          setCtError(e?.message || "Could not load container types.");
      } finally {
  ...
  const searchErrors = useMemo(() => {
    ...
    if (pickupDoorBlocked)
      errs.push(
        ...
        } does not support PICKUP at DOOR.`
      );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/seed/tariffs/get?${q}`, {
          cache: "no-store",
        });
        console.log("res data", res);
        if (!res.ok) throw new Error("Failed to load tariffs");
        const json = await res.json();
        console.log("Taffigs", json);
        ...
        // 🔧 normalize backend response into what UI expects
        const list: Tariff[] = rawList.flatMap((t: any) =>
          (t.rates || []).map((r: any) => ({
            id: t.id,
            containerTypeCode: (r.containerTypeCode || "").toUpperCase(), // 👈 use rate-level code
        ...
        const filtered = list.filter(
          (t) => allowed.includes(t.containerTypeCode) || true
        );
        ...
        if (!cancelled) {
          ...
          const pick =
            filtered.find((t) => t.containerTypeCode === primary)
              ?.containerTypeCode ||
        ...
        if (!cancelled)
          setTariffsError(e?.message ?? "Unable to fetch tariffs");
      } finally {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/seed/surcharges/get?${q}`, {
          cache: "no-store",
        });
        ...
        if (!cancelled)
          setSurchargesError(e?.message ?? "Unable to fetch surcharges");
      } finally {
    ...
    (async () => {
      ...
      setDepError(null);
      try {
        // Correct case for the API path
        const res = await fetch(
          `/api/seed/serviceschedules/get?page=1&limit=1000&includeVoyages=true`,
          { cache: "no-store" }
        );
        ...
        console.log("All service schedules:", items);

        if (!cancelled) {
          // Optionally map voyages as DepartureItems for your UI
          const departures: DepartureItem[] = items
            .flatMap((s: any) => s.voyages || [])
            .map((v: any) => ({
              date: v.departure
                ? new Date(v.departure).toISOString().slice(0, 10)
                : "",
              priceByType: v.priceByType || undefined,
  ...
  const selectedCode = (
    selectedCtCode || uiToPrimaryTariffCode(formData.containerType)
  ).toUpperCase();
  const selectedTariff: Tariff | null =
    tariffs.find(
      (t) => (t.containerTypeCode || "").toUpperCase() === selectedCode
    ) || null;
  ...
  function perContainerTotalFor(t: Tariff | null): number {
    ...
    for (const s of surcharges) {
      ...
      if (s.basis === "PERCENT" && s.percent)
        total += (t.amount * s.percent) / 100;
    }
    return total;
  }
  function grandTotalForSelection(t: Tariff | null): {
    currency: string;
    amount: number;
  } {
      ...
      <div>
          ...
          {[
            ...
            <React.Fragment key={step.num}>
              <div className="flex items-center gap-2">
                <div
                  className={clsx(
                    ...
                    currentStep >= step.num
                      ? "bg-white text-[#0F1B2A]"
                      : "bg-[#0F1B2A] text-[#faf9f6]"
                  )}
                >
                  {step.num}
                </div>
                <span
                  className={clsx(
                    currentStep >= step.num ? "text-white" : "text-[#faf9f6]",
                    "font-bold"
                  )}
                >
                  {step.label}
        ...
        {currentStep === 1 && (
          <div className={`${sectionStyle} p-10`} style={cardGradient}>
            ...
            <div className="mb-8">
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-2">
                <LocationInput
                  ...
                  onChange={(code, display, row) => {
                    ...
                    if (
                      ...
                      code.trim().toUpperCase() ===
                        formData.endLocation.trim().toUpperCase()
                    ) {
                  ...
                  excludeCodes={
                    formData.endLocation ? [formData.endLocation] : undefined
                  }
                ...
                <LocationInput
                  ...
                  onChange={(code, display, row) => {
                    ...
                    if (
                      ...
                      formData.startLocation &&
                      code.trim().toUpperCase() ===
                        formData.startLocation.trim().toUpperCase()
                    ) {
              ...
              <div className="grid md:grid-cols-2 gap-8 mt-6">
                {(["pickupType", "deliveryType"] as (keyof FormData)[]).map(
                  (field) => (
                    <div key={field}>
                      ...
                      <div className="space-y-2">
                        {(["door", "terminal"] as DeliveryType[]).map((opt) => (
                          <label
                            key={opt}
                            className="flex items-center gap-3 text-[#faf9f6] font-bold"
                          >
                            <input
                              type="radio"
                              name={field}
                              ...
                              onChange={() =>
                                handleInputChange(field as any, opt)
                              }
            ...
            <div className="mb-8">
              ...
              <label className="block text-[#faf9f6] font-light mb-2">
                VALID FROM
              </label>
              <input
                type="date"
            ...
            <div>
              ...
              <label className="flex items-center gap-3 text-[#faf9f6] font-light mb-6">
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("shipperOwnedContainer", e.target.checked)
                  }
              ...
              <div className="grid md:grid-cols-4 gap-4 mb-6">
                <div>
                  ...
                  >
                    ...
                    {!ctLoading && !containerTypes.length && (
                  ...
                  {ctError && (
                    <div className="mt-2 text-sm text-[#fa8a8a] font-bold">
                      {ctError}
                    </div>
                  )}
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("containerQuantity", e.target.value)
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("weightPerContainer", e.target.value)
                    }
                    className={`rounded-xl hover:text-[#00FFFF] text-white shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow border-black border-4 px-3 py-2 font-bold bg-[#11235d] hover:bg-[#1a307a] w-full`}
                  />
                </div>
                <div>
                  ...
                  <div className="flex gap-4 mt-2">
                    {(["kg", "lb"] as WeightUnit[]).map((u) => (
                      <label
                        key={u}
                        className="flex items-center gap-2 text-[#faf9f6] font-bold"
                      >
                        <input
                          type="radio"
                          name="weightUnit"
              ...
              <label className="flex items-center gap-3 text-[#faf9f6] font-light mb-4">
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("dangerousGoods", e.target.checked)
                  }
                  className="w-5 h-5 accent-[#00FFFF] bg-[#373737] border-2 border-black"
                />
                DANGEROUS GOODS
              ...
              {formData.dangerousGoods && (
                <div className="bg-[#290404] border-2 border-[#fa0404] rounded-lg p-6 mb-6">
                  <div className="flex items-center gap-2 mb-4 ">
                    ...
                    <h4 className="text-[#fa0404] font-bold text-lg">
                      DANGEROUS GOODS DETAILS
                    </h4>
                  </div>
                  <div className="grid md:grid-cols-2 gap-6 mb-4">
                    <div>
                      <label className="block text-[#faf9f6] font-bold mb-2">
                        IMO CLASS (OPTIONAL)
                      </label>
                      <select
                        value={formData.imoClass}
                        onChange={(e) =>
                          handleInputChange("imoClass", e.target.value)
                        }
                      ...
                      >
                        ...
                        <optgroup label="1. EXPLOSIVES">
                          ...
                          <option value="1.2">
                            1.2 Substances and articles which have a projection
                            hazard
                          </option>
                          <option value="1.3">
                        ...
                        <optgroup label="6. TOXIC AND INFECTIOUS SUBSTANCES">
                          ...
                          <option value="6.2">
                            6.2 Biohazard (Not Available)
                          </option>
                        </optgroup>
                        <optgroup label="7. RADIOACTIVE SUBSTANCES">
                          <option value="7">
                            7. Radioactive substances (Not Available)
                          </option>
                        </optgroup>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange("unNumber", e.target.value)
                        }
                        className={`rounded-xl hover:text-white text-white shadow-[4px_4px_0px_rgba(0,0,0,1)] hover:shadow-[10px_8px_0px_rgba(0,0,0,1)] transition-shadow border-black border-4 px-3 py-2 font-bold bg-[#fa0404] hover:bg-[#d93838] w-full font-bold`}
              ...
              <div className="flex justify-center">
                <button
                  ...
                  title={
                    !canProceedToNext()
                      ? searchErrors[0] || "Complete routing to continue"
                      : undefined
                  }
                >
                  SEARCH
          ...
          <div className={`${sectionStyle} p-13 mb-13`} style={cardGradient}>
            ...
            <div className="mb-6">
              <div className="p-4 rounded-2xl flex items-center justify-between">
                {[
                  {
                    name: startLabel || formData.startLocation,
                    type: formData.pickupType,
                  },
                  {
                    name: endLabel || formData.endLocation,
            ...
            <div className="mb-8">
              ...
              <div className="flex items-center gap-4 overflow-x-auto pb-4">
                <button
                  ...
                  onClick={() =>
                    setSelectedDepartureIdx((i) => Math.max(0, i - 1))
                  }
                ...
                {(departures.length
                  ...
                  const priceObj =
                    ...
                    (tariffs.find(
                      (t) =>
                        (t.containerTypeCode || "").toUpperCase() ===
                        selectedCode
                    )
                      ? {
                          currency: (
                    ...
                    >
                      ...
                      >
                        {priceObj
                          ? `${
                              priceObj.currency
                            } ${priceObj.amount.toLocaleString()}`
                          : "—"}
                      </div>
                ...
                <button
                  className="p-2 text-[#22D3EE] hover:text-white transition-colors disabled:opacity-40"
                  onClick={() =>
                    setSelectedDepartureIdx((i) =>
                      Math.min((departures.length || 1) - 1, i + 1)
                    )
                  }
                  disabled={
                    selectedDepartureIdx >= (departures.length || 1) - 1
                  }
                  aria-label="Next"
                >
                ...
                <div className="p-6 bg-[#0A1A2F]">
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-6">
                    {allowed.map((ct) => {
                      const t = tariffs.find(
                        (x) => (x.containerTypeCode || "").toUpperCase() === ct
                      );
                      return (
                        <button
                          key={ct}
                  ...
                  <div className="space-y-3">
                    {allowed.map((code) => {
                      const t = tariffs.find(
                        (x) =>
                          (x.containerTypeCode || "").toUpperCase() === code
                      );
                      return (
                        <div
                          key={code}
                          className="flex justify-between items-center bg-[#1A2A4A] p-3 rounded-lg border border-[#2D4D8B]"
                        >
                          <div className="flex items-center gap-3">
                  ...
                  <div className="mt-6 space-y-3 text-sm">
                    <div className="flex items-center gap-3 text-[#22D3EE]">
                      ...
                      <span className="text-white">
                        Equipment and Loading guarantee
                      </span>
                    </div>
                    <div className="flex items-center gap-3 text-[#22D3EE]">
                      <CheckCircle size={16} />
                    ...
                    <div className="flex items-center gap-3 text-[#22D3EE]">
                      <DollarSign size={16} />
                      <span className="text-white">
                        Fixed Ocean Freight (Surcharges valid at time of
                        shipment)
                      </span>
                    </div>
                  </div>
                      ...
                      <span className="font-bold text-[#22D3EE]">
                        {selectedTariff
                          ? `${
                            ...
                            } ${selectedTariff.amount.toLocaleString()}`
                          : "—"}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between text-base font-bold">
                      ...
                      <span className="text-[#00FFFF]">
                        {(() => {
                          ...
                          return `${g.currency} ${g.amount.toLocaleString(
                            undefined,
                ...
                <div className="bg-[#1A2A4A] rounded-2xl border-2 border-[#22D3EE] shadow-[12px_12px_0px_rgba(0,0,0,1)]">
                  ...
                  <div className="p-4 text-white space-y-3">
                    <div className="text-xs opacity-80">
                      For departure:{" "}
                      <span className="font-bold text-[#22D3EE]">
                        {fmtDateTime(selectedDepartureDate)}
                      </span>
                    </div>
                    <div className="space-y-2">
                      {cutoffs.map((m, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            className="flex items-center justify-between bg-[#0F1B2A] border border-[#2D4D8B] rounded-xl px-3 py-2"
                          >
                  ...
                  <div className="p-4 text-white text-sm space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold">
                        {formData.shipperOwnedContainer ? "Yes" : "No"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>Commodity</span>
                      ...
                      <div>
                        ...
                        <div className="bg-[#2D4D8B] border-4 border-black p-4 mb-6 shadow-[10px_10px_0px_rgba(0,0,0,1)] rounded-xl">
                          ...
                          <div className="text-xs mt-2 opacity-90 text-white/90">
                            Includes base ocean freight + any per-container /
                            percent surcharges. Per-shipment charges are added
                            once at checkout.
                          </div>
                        </div>
                            ...
                            <div className="bg-[#051241] border-4 border-black p-4 mb-6 shadow-[10px_10px_0px_rgba(0,0,0,1)] rounded-xl">
                              <div className="grid grid-cols-3 gap-4 text-sm">
                                ...
                                {surcharges.map((s, i) => {
                                  let text = "-";
                                  if (
                                    s.basis === "PER_CONTAINER" &&
                                    s.amount != null
                                  ) {
                                    text = `${
                                      s.currency
                                    } ${s.amount.toLocaleString()} / container`;
                                  } else if (
                                    s.basis === "PER_SHIPMENT" &&
                                    s.amount != null
                                  ) {
                                    text = `${
                                      s.currency
                                    } ${s.amount.toLocaleString()} per shipment`;
                                  } else if (
                                    s.basis === "PERCENT" &&
                                    s.percent != null
                                  ) {
                                  ...
                                  return (
                                    <React.Fragment key={i}>
                                      <div title={s.description || s.code}>
                                        {s.code}
                                      </div>
                            ...
                            <div className="bg-[#0A1A2F] border-4 border-white p-4 rounded-3xl shadow-[30px_30px_0px_rgba(0,0,0,1)]">
                              {(() => {
                                const { currency, amount } =
                                  grandTotalForSelection(selectedTariff);
                                return (
                                  <>
                                    <div className="flex justify-between text-xl font-bold">
                                      <span className="text-[#00FFFF]">
                                        TOTAL ESTIMATE ({qty}×{" "}
                                        {selectedTariff.containerTypeCode}):
                                      </span>
                                      <span className="text-[#00FFFF]">
                                        {currency}{" "}
                                    ...
                                    <div className="text-xs opacity-90 mt-1">
                                      Includes per-shipment charges once;
                                      actuals may vary at booking.
                                    </div>
                                  </>
                                );
                              })()}
                    ...
                    {showModal === "remarks" && (
                      <div>
                        ...
                        <div className="bg-[#1A2A4A] rounded-xl border-4 border-black p-6 mb-6">
                          <p className="mb-4">
                            Future Marine Fuel Recovery (MFR) surcharge
                            adjustments may not be reflected. Check your local
                            tariff notes and terms for full validity and
                            exceptions.
                          </p>
                          <p>
                            Detention & Demurrage and country-specific fees may
                            apply depending on equipment and terminal handling.
                          </p>
                        </div>
                      </div>
                    )}
        ...
        {currentStep === 3 &&
          (() => {
            ...
            const totals = t
              ? grandTotalForSelection(t)
              : { currency: "USD", amount: 0 };
            const qty = Math.max(1, Number(formData.containerQuantity || 1));
            return (
              <div
                className={`${sectionStyle} p-10 mb-13`}
                style={cardGradient}
              >
                <h2 className="text-xl font-bold text-[#faf9f6] mb-6 flex items-center gap-2">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-8 ">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[#00FFFF] uppercase">
                          {startLabel || formData.startLocation || "Start"}
                        </span>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="uppercase text-[#00FFFF]">
                          {endLabel || formData.endLocation || "End"}
                        </span>
                    ...
                    <div className="border-l border-[#00FFFF] pl-6 flex items-center gap-6">
                      <div className="text-center">
                        ...
                        <div className="uppercase text-[#00FFFF] text-sm font-bold">
                          {qty}×{" "}
                          {selectedCtCode ||
                            uiToPrimaryTariffCode(formData.containerType)}
                        </div>
                      </div>
                      <div className="text-center">
                ...
                <div
                  className="bg-[#22546d] border-4 border-white rounded-2xl p-10 mb-10 shadow-[30px_30px_0px_rgba(0,0,0,1)]"
                  style={cardGradient}
                >
                    ...
                    <div className="bg-[#22546d] rounded-3xl border-2 border-white p-4 mb-10 shadow-[30px_30px_0px_rgba(0,0,0,1)]">
                      ...
                      <div className="flex flex-col gap-2 text-sm text-[#faf9f6]">
                        <div className="flex justify-between w-full">
                          <span>
                            Base Ocean Freight ({t.containerTypeCode})
                          </span>
                ...
                <div className="bg-[#071e3d] rounded-2xl mb-10 border-4 border-white p-6 shadow-[30px_30px_0px_rgba(0,0,0,1)]">
                  ...
                  <div className="flex gap-4 mt-6">
                    <button
                      className={`border-2 rounded-3xl text-white px-4 mr-2 py-3 bg-[#2a72dc] hover:bg-[#071e3d] hover:text-[#00FFFF] flex items-center gap-2 shadow-[10px_10px_0px_rgba(0,0,0,1)] hover:shadow-[15px_15px_0px_rgba(0,0,0,1)] transition-shadow`}
                    >
                      <Send size={20} /> SEND QUOTE REQUEST
                    </button>
                    <button
                      className={`border-2 rounded-3xl text-white px-3 mr-2 py-2 bg-[#2a72dc] hover:bg-[#071e3d] hover:text-[#00FFFF] flex items-center gap-2 shadow-[10px_10px_0px_rgba(0,0,0,1)] hover:shadow-[15px_15px_0px_rgba(0,0,0,1)] transition-shadow`}
                    >
